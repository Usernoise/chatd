...
ФОРМАТ ОТВЕТА (строго JSON):
{
    ...
    "genre": "Жанр (Pop, Rock, Rap, Electronic, Jazz, Classical или любой другой, важно определить подходящий)",
    "mood": "Настроение (Happy, Sad, Energetic, Calm, Funny, Dramatic или любой другой, важно определить подходящий)",
...
def generate_music_with_suno(song_data):
    ...
    try:
        ...
        # Сохраняем улучшенный текст, чтобы отобразить его в сообщении
        try:
            song_data['lyrics_improved'] = improved_lyrics if improved_lyrics else song_data.get('lyrics', '')
        except Exception:
            # На всякий случай не падаем, если song_data не изменяемый
            pass
...
def format_song_message(song_data):
    ...
    # Показываем улучшенный текст, если он есть
    lyrics_to_show = song_data.get('lyrics_improved') or song_data.get('lyrics', '')

    message = f"""🎵 <b>ПЕСНЯ ДНЯ</b> 🎵

🎼 <b>Название:</b> {song_data.get('song_title', 'Не указано')}
🎭 <b>Жанр:</b> {song_data.get('genre', 'Не указано')}
😊 <b>Настроение:</b> {song_data.get('mood', 'Не указано')}

📝 <b>Описание событий:</b>
{song_data.get('description', 'Не указано')}

👥 <b>Главные герои:</b>
{', '.join(song_data.get('main_characters', []))}

🎯 <b>Ключевые события:</b>
{', '.join(song_data.get('key_events', []))}
